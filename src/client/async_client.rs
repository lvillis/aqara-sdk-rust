...
use crate::api;
use crate::auth::SignatureParts;
use crate::auth::sign_headers;
...
use crate::types::{
    ...
    RetryConfig, SecretString, TimeoutConfig,
};
...
impl Client {
    ...
    pub(crate) fn from_builder(builder: ClientBuilder) -> Result<Self> {
        ...
        let transport =
            AsyncTransport::new(cfg.timeouts.connect).map_err(|e| Error::InvalidConfig {
                message: format!("failed to build async http client: {e}"),
            })?;
